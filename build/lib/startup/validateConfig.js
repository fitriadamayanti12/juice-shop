...
const validateConfig = async ({ products, memories, exitOnFailure = true }) => {
    ...
    if (success) {
        logger_1.default.info(`Configuration ${safe_1.default.bold(process_1.default.env.NODE_ENV ?? 'default')} validated (${safe_1.default.green('OK')})`);
    }
    else {
        logger_1.default.warn(`Configuration ${safe_1.default.bold(process_1.default.env.NODE_ENV ?? 'default')} validated (${safe_1.default.red('NOT OK')})`);
...
const checkYamlSchema = (configuration = config_1.default.util.toObject()) => {
    let success = true;
    const schemaErrors = (0, src_1.default)(configuration, { schemaPath: path_1.default.resolve('config.schema.yml'), logLevel: 'none' });
